...
class Embedding_based(nn.Module):
    ...
    def calc_kg_loss_TransR(self, h, r, pos_t, neg_t):
        ...
        kg_loss = -torch.log(torch.sigmoid(pos_score - neg_score)).mean()
    ...
    def calc_kg_loss_TransE(self, h, r, pos_t, neg_t):
        ...
        r_embed = torch.div(r_embed, torch.norm(r_embed, p=2, dim=-1))
        h_embed = torch.div(h_embed, torch.norm(h_embed, p=2, dim=-1))
        ...
        pos_score = torch.norm(h_embed + r_embed - pos_t_embed, p=1, dim=-1, keepdim=True)                                                                     # (kg_batch_size)
        neg_score = torch.norm(h_embed + r_embed - neg_t_embed, p=1, dim=-1, keepdim=True)                                                                     # (kg_batch_size)
